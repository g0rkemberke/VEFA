...
import { X, MapPin, Check, Plus, CreditCard, Info, Map, AlertCircle, ChevronRight, ChevronLeft, CheckCircle, User, LocateFixed } from 'lucide-react';

// MERKEZİ İSTANBUL İLÇELERİ LİSTESİ (Sıfır Hata Prensibi)
...
const OrderDrawer = ({
  ...
  const handleNextStep = () => {
    if (step === 1) {
      // YENİ: graveDesign.ilce de kontrol ediliyor
      if (!graveDesign?.name?.trim() || !graveDesign?.ilce?.trim() || !graveDesign?.cemetery?.trim() || !graveDesign?.ada?.trim() || !graveDesign?.parsel?.trim()) {
        setError('Lütfen işleme devam etmeden önce tüm alanları (İlçe dahil) eksiksiz doldurun.');
        return;
      }
    }
    setError('');
          ...
          {step === 1 && (
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
              ...
              <div className="space-y-5">
                ...
                {/* YENİ: İLÇE SEÇİM ALANI */}
                <div>
                  <label className="text-[10px] font-bold text-[#134B36]/60 uppercase tracking-widest mb-2 block">
                    Mezarlığın Bulunduğu İlçe
                  </label>
                  <div className="relative">
                    <select
                      value={graveDesign?.ilce || ''}
                      onChange={(e) => {
                        setGraveDesign({ ...graveDesign, ilce: e.target.value });
                        if (error) setError('');
                      }}
                      className={`w-full pl-11 pr-4 py-4 bg-white border ${error && !graveDesign?.ilce?.trim() ? 'border-red-400 shadow-[0_0_0_4px_rgba(248,113,113,0.1)]' : 'border-[#134B36]/10 focus:border-[#C9A84C]'} rounded-xl outline-none focus:ring-4 focus:ring-[#C9A84C]/10 transition-all text-[#1a1c19] text-sm font-medium appearance-none cursor-pointer`}
                    >
                      <option value="" disabled>Lütfen İlçe Seçiniz</option>
                      {ISTANBUL_ILCELERI.map(ilce => (
                        <option key={ilce} value={ilce}>{ilce}</option>
                      ))}
                    </select>
                    <LocateFixed size={18} className={`absolute left-4 top-1/2 -translate-y-1/2 ${error && !graveDesign?.ilce?.trim() ? 'text-red-400' : 'text-[#134B36]/40'}`} />
                  </div>
                </div>
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-4 rounded-2xl border text-left transition-all duration-300 flex items-start gap-4 ${isSelected ? 'border-[#134B36] bg-[#134B36]/5 shadow-[0_5px_15px_rgba(19,75,54,0.05)]' : 'border-[#134B36]/10 bg-white hover:border-[#134B36]/30'
                        }`}
                    >
