              ...
              {latestOrder && (
                <div className="bg-white rounded-[2.5rem] p-8 border border-[#134B36]/10 shadow-sm mt-8">
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute top-6 left-6 h-1 bg-[#C9A84C] rounded-full z-0 hidden md:block transition-all duration-1000 ease-in-out" style={{ width: latestOrder.status === 'pending' || latestOrder.status === 'awaiting_partner' ? '0%' : latestOrder.status === 'in-progress' ? '50%' : '100%' }}></div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
                      <div className="flex flex-col items-center text-center">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center border-4 border-white shadow-md mb-3 transition-colors ${latestOrder.status === 'pending' || latestOrder.status === 'awaiting_partner' || latestOrder.status === 'in-progress' ? 'bg-[#C9A84C] text-[#0d1a10]' : 'bg-gray-200 text-gray-500'}`}><CheckSquare size={20} /></div>
                        <h4 className={`text-sm font-bold ${latestOrder.status === 'pending' || latestOrder.status === 'awaiting_partner' || latestOrder.status === 'in-progress' ? 'text-[#134B36]' : 'text-gray-400'}`}>Sipariş Alındı</h4>
                      </div>
                      <div className="flex flex-col items-center text-center">
                  ...
                  {/* YENİ: GİZLİLİK DUVARLI USTA BİLGİSİ */}
                  <div className="mt-8 pt-6 border-t border-[#134B36]/5">
                    <div className="flex items-center justify-between p-4 bg-[#F8F6F0] rounded-xl border border-[#134B36]/5">
                      <div className="flex items-center gap-3">
                        <User size={18} className="text-[#134B36]/50" />
                        <div>
                          <p className="text-[10px] uppercase font-black tracking-widest text-[#134B36]/50">Sorumlu Usta</p>
                          <p className="text-sm font-bold text-[#134B36]">{latestOrder.assignedPartnerName || 'Sistem Ataması Bekleniyor'}</p>
                        </div>
                      </div>
                      {latestOrder.assignedPartnerName && latestOrder.assignedPartnerName !== 'Atama Bekleniyor' && (
                        <span className="text-[9px] font-bold text-[#134B36]/40 uppercase tracking-widest bg-white px-2 py-1 rounded-md border border-[#134B36]/10">
                          İletişim Gizli
                        </span>
                      )}
                    </div>
